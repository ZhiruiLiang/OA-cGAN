...
function load_data(hparams)
    ...
    #labels = Matrix(CSV.read("data/data_load/labels.csv", DataFrame, header=false))
    ...
    #data_train = DataLoader((data_tensor[:,:,:,1:500], y[:,1:500], DA_load[:,:,1:500]), batchsize=hparams.batch_size, shuffle=true)
    #data_test = DataLoader((data_tensor[:,:,:,501:550], y[:,501:550], DA_load[:,:,501:550]), batchsize=hparams.batch_size, shuffle=true)
...
function generator_loss(fake_output)
    gen_loss = logitbinarycrossentropy(fake_output, 0f0)
    return gen_loss
end
...
function train_gen(gen, discr, original_data, original_label, original_load, opt_gen, opt_discr, hparams, dt)
    ...
    gs_gen = gradient(() -> generator_loss(discr(fake_y, gen(fake_y, noise))), ps_gen)
    update!(opt_gen, ps_gen, gs_gen)

    loss = Dict()
    loss["discr"] = train_discr(discr, fake_data, fake_y, original_data, original_label, opt_discr)
    loss["gen"] = generator_loss(discr(fake_y, fake_data))
    return loss
end
...
function train_cGAN()
        ...
        for (x, y, z) in data_train
            ...
            if train_steps % hparams.verbose_freq == 0
                ...
                for i in 0:hparams.nclasses-1
                    ...
                    a = zeros(hparams.data_rows,hparams.data_cols*1000)
                    a[:,1:hparams.data_cols] = test_output
                    final_results = StatsBase.reconstruct(dt::ZScoreTransform, a)
                    draw_pictures(ep, final_results[:,1:hparams.data_cols], hparams, i)
                end
            end
            train_steps += 1
        end
    ...
    save_loss(loss_gen, loss_discr, total_cost, hparams)
end
...
function test_G(ep, gen, hparams, test_error, test_labels, original_load, dt)
    ...
    CSV.write("output/output_Normal_OPF/test/Error_$ep.csv", Tables.table(gen_error_piled), header=false)
    return OPF_cost
end
...
function draw_pictures(datadir, test_output, hparams, i)
    ...
    plotd = plot(plotd1,plotd2,plotd3,plotd4,plotd5,plotd6,plotd7,plotd8,plotd9,plotd10,plotd11,plotd12,
                 layout = (hparams.output_x, hparams.output_y), legend = false, fmt = :png)
    savefig(plotd,"output/output_Normal_OPF/label_$i/$datadir.png")
    CSV.write("output/output_Normal_OPF/label_$i/Power_$datadir.csv", Tables.table(test_output), header=false)
end

function save_loss(loss_gen, loss_discr, total_cost, hparams)
    e_list = collect(1:hparams.epochs)
    f = open("output/output_Normal_OPF/loss_gen.txt", "w")
    for e in e_list
        println(f, loss_gen[e])
    end
    close(f)
    f = open("output/output_Normal_OPF/loss_discr.txt", "w")
    for e in e_list
